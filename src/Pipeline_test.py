        ...
        for c in range(chunks_amount):
            ...
            if 0 in self.ranks: # begin of the pipeline (first layer)
                ...
                if self.backend == 'gloo':
                    distops.send(tensor=out.cpu(), dst=next_rank[0]) # send the tensor
                else:
                    distops.send(tensor=out, dst=next_rank[0])
                ...
                temp = torch.empty(*self.shapes[i][0](chunk_shapes[c]), device='cpu' if self.backend == 'gloo' else f'cuda:{self.gpu_id}', requires_grad=True)
                ...
                self.outputs = self.layers[0](temp)#torch.cat((self.outputs, self.layers[0](temp)), dim=0)
    ...
    def backward(self, loss):
        ...
        if self.rank in self.rank_list[-1]: # End of the pipeline
            if len(self.rank_list[-1])==1:
                ...
                grad_data = autograd.grad(self.outputs, self.inputs, grad_outputs=grad_output, retain_graph=True)[0] # this is needed to compute the derivative at the previous layer
                ...
                for param in self.layers[-1].parameters():
                    param.grad = autograd.grad(self.outputs, param, grad_outputs=grad_output, retain_graph=True)[0]      
            else:
                raise NotImplementedError("Tensor parallelism is not yet implemented.")
...
def main(rank=None, master_addr=None, master_port=None, world_size=None):
    ...
    # gather the gradients of model from rank 1 to rank 0
    if rank == 1:
        model_grads = model.grad()
        model_grads = [grad.cpu().flatten() for grad in model_grads]
        dist.send(torch.cat(model_grads), dst=0)
    else:
        grad_layer2 = torch.cat([torch.zeros_like(param.flatten()) for i,param in enumerate(Model.parameters()) if i>=4]).cpu()
        dist.recv(grad_layer2, src=1)
        grad_layer1 = [param.grad.flatten() for param in model.parameters()][:4]
        model_grads = torch.cat(grad_layer1+[grad_layer2.to('cuda:0')])
    
        torch.manual_seed(0)
        x = torch.randn(10, 100).to('cuda:0')
        torch.manual_seed(0)
        target = torch.randint(0, 50, (10,50), dtype=torch.float32).to('cuda:0')
        Model = Model.to('cuda:0')
        output = criteria(Model(x), target)
        output.backward()
        Model_grads = torch.cat([param.grad.flatten() for param in Model.parameters()])

        print(torch.norm(Model_grads-model_grads))
    
    if rank == 0:
        print(torch.norm(torch.cat([param.grad.flatten() for param in Model.parameters()]) - torch.cat([param.grad.flatten() for param in model.parameters()])))
